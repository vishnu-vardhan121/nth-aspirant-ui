-- Key skills / tags for landing job cards (admin-editable text array).

alter table public.jobs
  add column if not exists key_skills text[] not null default '{}';

comment on column public.jobs.key_skills is
  'Short skill tags shown on the public landing job section (e.g. Linux, AWS, Docker). Comma-separated in admin UI.';

-- Spotlight RPC: include key_skills for spotlight-only cards on landing.
...
comment on function public.get_landing_hiring_spotlight() is
  'Anonymous: { jobs: [...] } for hiring_spotlight roles; includes key_skills, application_deadline, job_status, notices, shortlisted.';
