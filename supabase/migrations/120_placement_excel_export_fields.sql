-- Placement Excel export: return education + resume paths for aspirant IDs (admin only).

create or replace function public.get_aspirant_placement_export_fields(p_ids uuid[])
returns jsonb
language plpgsql
security definer
...
comment on function public.get_aspirant_placement_export_fields(uuid[]) is
  'Admin: fields needed for placement Excel export (education JSON, resume path, college/tier).';

grant execute on function public.get_aspirant_placement_export_fields(uuid[]) to authenticated;

-- Job applicants list: add college / qualification / marks columns for placement export.
...
comment on function public.get_job_applications(
  uuid, text[], text, text, numeric, numeric, numeric, text, numeric, numeric, text, text
) is 'Admin: job applicants with plan, placement, mock filters, and placement-export profile fields.';
