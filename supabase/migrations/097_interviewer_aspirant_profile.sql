-- Interviewer: view aspirant profile + mock history for assigned mock registrations only.

-- Resume read for aspirants the interviewer is (or was) assigned to mock.
...
begin
  ...
  where r.id = p_mock_registration_id
    and r.interviewer_id = v_uid;

  if v_aspirant_id is null then
    return jsonb_build_object('ok', false, 'error', 'Mock not found');
  end if;
...
comment on function public.get_aspirant_profile_for_interviewer(uuid) is
  'Interviewer: aspirant profile + mock history when assigned to the given mock registration.';

grant execute on function public.get_aspirant_profile_for_interviewer(uuid) to authenticated;
