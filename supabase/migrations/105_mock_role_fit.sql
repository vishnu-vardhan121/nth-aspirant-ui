-- Optional interviewer-only role-fit tags on mock feedback (admin filtering; hidden from aspirants).

alter table public.mock_registrations
  add column if not exists role_fit_keys text[] default '{}';

create index if not exists mock_registrations_role_fit_keys_gin
  on public.mock_registrations using gin (role_fit_keys);

comment on column public.mock_registrations.role_fit_keys is
  'Interviewer-only: roles/stacks the candidate is a good fit for (admin filters). Not shown to aspirants.';

-- Sanitize role-fit keys from frontend (trim, lowercase, dedupe). Options live in the UI only.
create or replace function public.normalize_mock_role_fit_keys(p_keys jsonb)
returns text[]
language plpgsql
immutable
as $$
...
begin
  ...
  for v_key in
    ...
    where length(trim(value)) > 0 and length(trim(value)) <= 60
  loop
    if not (v_key = any (v_out)) then
      v_out := array_append(v_out, v_key);
    end if;
  end loop;
...
begin
  ...
  v_tech_feedback := p_tech_feedback - 'role_fit';
...
comment on function public.submit_mock_feedback(uuid, int, int, text, jsonb) is
  'Interviewer/admin: topic feedback (up to 10) + optional internal role-fit tags.';

-- Admin users list: filter by role-fit tag from any completed mock.
...
) is 'Admin: aspirant list with profile, mock, topic, and role-fit filters.';

grant execute on function public.get_admin_users_list(
  text, text, text, text, text, text, int, numeric, text, text, text, text, text, text, numeric, numeric, numeric, text, numeric, numeric, text, text, int, int
) to authenticated;

-- Admin profile: include role_fit_keys on mock history.
...
comment on function public.get_aspirant_profile_for_admin is
  'Admin: full aspirant profile with mock history (includes internal role-fit tags).';
