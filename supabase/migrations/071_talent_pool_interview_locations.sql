-- Talent pool: optional flags for in-person interview availability (Hyderabad / Bangalore).

alter table public.talent_pool_candidates
  add column if not exists ready_interview_hyderabad boolean not null default false,
  add column if not exists ready_interview_bangalore boolean not null default false;

comment on column public.talent_pool_candidates.ready_interview_hyderabad is
  'Candidate is ready to attend an interview in Hyderabad.';
comment on column public.talent_pool_candidates.ready_interview_bangalore is
  'Candidate is ready to attend an interview in Bangalore.';
...
create or replace function public.submit_talent_pool_candidate(
  ...
  p_ready_interview_hyderabad boolean default false,
  p_ready_interview_bangalore boolean default false,
...
begin
  ...
  insert into public.talent_pool_candidates (
    ...
    ready_interview_hyderabad, ready_interview_bangalore,
    ...
    coalesce(p_ready_interview_hyderabad, false),
    coalesce(p_ready_interview_bangalore, false),
    p_years_experience,
...
grant execute on function public.submit_talent_pool_candidate(
  text, text, text, text, text, boolean, numeric, boolean, text,
  text[], text[], text, text, numeric, numeric, numeric, date, text, text, text,
  integer, boolean, text, boolean, boolean, text
