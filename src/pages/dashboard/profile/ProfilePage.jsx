...
export default function ProfilePage() {
  ...
  const handleReplaceResume = async (e) => {
    e.preventDefault();
    setMessage({ type: '', text: '' });
    if (!user?.id || !resumeReplacementFile) return;
    setReplacingResume(true);

    const oldPath = profile?.resume_url || null;
    const ext = resumeReplacementFile.name.split('.').pop()?.toLowerCase() || 'pdf';
    const newPath = `${user.id}/resume.${ext}`;

    if (oldPath) {
      const { error: delError } = await supabase.storage.from('resumes').remove([oldPath]);
      if (delError) {
        setMessage({ type: 'error', text: delError.message ?? 'Failed to remove old resume.' });
    ...
    if (updateError) {
      setMessage({ type: 'error', text: updateError.message ?? 'Failed to update resume link.' });
      setReplacingResume(false);
      return;
    }
    dispatch(setAspirantProfile(data));
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Manage your personal details, education, skills, and resume. Keep this up to date for better job matches.
        </p>
      </div>
        ...
        <form onSubmit={handleSaveProfile} className="space-y-4">
          ...
          <div className="pt-2">
            <button
              ...
              className="nth-btn-primary px-5 py-2.5 rounded-lg text-sm font-semibold disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none"
            >
              {saving ? <ButtonLoader label="Saving…" /> : 'Save education'}
            </button>
          </div>
        </form>
      </SectionCard>
      ...
      >
        <form onSubmit={(e) => { e.preventDefault(); handleSaveProfile(e); }} className="space-y-4">
          <div>
          ...
          <div className="pt-2">
            <button
              ...
              className="nth-btn-primary px-5 py-2.5 rounded-lg text-sm font-semibold disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none"
            >
              {saving ? <ButtonLoader label="Saving…" /> : 'Save skills'}
            </button>
          </div>
        </form>
      </SectionCard>
      ...
      <SectionCard
        ...
        title="Resume"
        subtitle="One resume per account. Uploading a new file replaces the previous one."
      >
        <div className="space-y-4">
          {resumeSignedUrl ? (
            ...
            <p className="text-sm text-slate-500">No resume uploaded yet. Add one below to share with recruiters.</p>
          )}

          <form onSubmit={handleReplaceResume} className="flex flex-col sm:flex-row sm:items-end gap-4">
            <div className="flex-1 min-w-0">
              <label className={labelClass}>Replace or upload resume (PDF, DOC, DOCX)</label>
              <input
                type="file"
                accept=".pdf,.doc,.docx"
            ...
            <button
              type="submit"
              disabled={!resumeReplacementFile || replacingResume}
              className="shrink-0 inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-slate-200 bg-white text-slate-700 font-medium hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white transition-colors"
            >
              <HiArrowDownTray className="h-4 w-4" />
              {replacingResume ? 'Uploading…' : 'Upload resume'}
            </button>
          </form>
        </div>
      </SectionCard>
